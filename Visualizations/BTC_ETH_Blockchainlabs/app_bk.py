...
st.set_page_config(page_title="Seguimiento Histórico de Bitcoin y Ethereum", page_icon="📈", initial_sidebar_state="collapsed", layout="wide")
...
st.title("Seguimiento Histórico de Bitcoin y Ethereum")
...
with col1:
    ...
    if not df_btc.empty:
        ...
        st.write(f"Cambio 1h: {df_btc['btc_percent_change_1h'].iloc[-1]:.2f}% {'🟢' if df_btc['btc_percent_change_1h'].iloc[-1] >= 0 else '🔴'}", unsafe_allow_html=True)
        st.write(f"Cambio 24h: {df_btc['btc_percent_change_24h'].iloc[-1]:.2f}% {'🟢' if df_btc['btc_percent_change_24h'].iloc[-1] >= 0 else '🔴'}", unsafe_allow_html=True)
        st.write(f"Cambio 7d: {df_btc['btc_percent_change_7d'].iloc[-1]:.2f}% {'🟢' if df_btc['btc_percent_change_7d'].iloc[-1] >= 0 else '🔴'}", unsafe_allow_html=True)
...
with col2:
    ...
    if not df_eth.empty:
        ...
        st.write(f"Cambio 1h: {df_eth['eth_percent_change_1h'].iloc[-1]:.2f}% {'🟢' if df_eth['eth_percent_change_1h'].iloc[-1] >= 0 else '🔴'}", unsafe_allow_html=True)
        st.write(f"Cambio 24h: {df_eth['eth_percent_change_24h'].iloc[-1]:.2f}% {'🟢' if df_eth['eth_percent_change_24h'].iloc[-1] >= 0 else '🔴'}", unsafe_allow_html=True)
        st.write(f"Cambio 7d: {df_eth['eth_percent_change_7d'].iloc[-1]:.2f}% {'🟢' if df_eth['eth_percent_change_7d'].iloc[-1] >= 0 else '🔴'}", unsafe_allow_html=True)
