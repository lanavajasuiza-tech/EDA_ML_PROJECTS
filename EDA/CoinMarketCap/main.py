...
# Split data into three periods: 2010-2021, 2021-2023, and 2023+
df_2010_2021 = filtered_df[(filtered_df['dateAdded'] >= '2010-01-01') & (filtered_df['dateAdded'] < '2021-01-01')]
df_2021_2023 = filtered_df[(filtered_df['dateAdded'] >= '2021-01-01') & (filtered_df['dateAdded'] < '2023-01-01')]
df_2023_onward = filtered_df[filtered_df['dateAdded'] >= '2023-01-01']

# Create subplots in a single window
...
axes[0].set_title('Cryptocurrency Max Supply (2010-2021)', fontsize=14)
axes[0].set_ylabel('Max Supply (Jittered)', fontsize=12)
...
axes[1].set_title('Cryptocurrency Max Supply (2021-2023)', fontsize=14)
axes[1].set_ylabel('Max Supply (Jittered)', fontsize=12)
...
axes[2].set_title('Cryptocurrency Max Supply (2023 Onward)', fontsize=14)
axes[2].set_xlabel('Date Added', fontsize=12)
axes[2].set_ylabel('Max Supply (Jittered)', fontsize=12)
axes[2].grid(axis='y', linestyle='--', alpha=0.7)

# Adjust color bar
norm = plt.Normalize(filtered_df['price'].min(), filtered_df['price'].max())
sm = plt.cm.ScalarMappable(cmap="viridis", norm=norm)
sm.set_array([])
cbar = fig.colorbar(sm, ax=axes, orientation='vertical', label='Price ($)', fraction=0.02, pad=0.08)

# Adjust layout for better spacing
plt.tight_layout()
plt.subplots_adjust(right=0.85)  # Leave space for the color bar on the right
plt.show()
